  ...
  wiredLogs(result) {
    ...
    if (data) {
      ...
      tempCols = tempCols.filter(
        (col) =>
          col.fieldName !== "Name" && col.fieldName !== "QuickBridgeTLG__Name"
      );
      ...
      let processedData = data.records.map((record) => {
        ...
        data.columns.forEach((col) => {
          if (col.type === "url") {
            ...
            if (newRec[originalField]) {
              newRec[col.fieldName] =
                `/lightning/r/${newRec[originalField]}/view`;
              ...
              newRec[originalField + "_Name"] =
                newRec[relationName] && newRec[relationName].Name
                  ? newRec[relationName].Name
                  : newRec[originalField];
            } else {
              newRec[col.fieldName] = "";
              newRec[originalField + "_Name"] = "";
            }
          ...
          if (
            col.fieldName !== "Name" &&
            col.fieldName !== "QuickBridgeTLG__Name"
          ) {
            let isUrl = col.type === "url";
            let isDate = col.type === "date";
            let displayValue = isUrl
              ? newRec[col.fieldName.replace("_Url", "") + "_Name"]
              : newRec[col.fieldName];
            mobileFields.push({
              label: col.label,
              value: displayValue,
      ...
      if (this.isRetryableGateway) {
        ...
        this.freshLogs = processedData.filter(
          (rec) => (rec.QuickBridgeTLG__Retry_Count__c || 0) === 0
        );

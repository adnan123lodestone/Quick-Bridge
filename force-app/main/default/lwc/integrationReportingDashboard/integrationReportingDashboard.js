...
export default class IntegrationReportingDashboard extends LightningElement {
  ...
  get selectedProduct() {
    return this.products.find(
      (product) => product.productKey === this.normalizedSelectedProductKey
    );
  }
  ...
  get availableProducts() {
    ...
    return this.products.filter(
      (product) => !this.isSubscribedProduct(product)
    );
  }
  ...
  async loadSummary() {
    ...
    try {
      ...
      this.errorMessage =
        error?.body?.message ||
        error?.message ||
        "Could not load reporting metrics.";
    } finally {
  ...
  decorateProduct(row) {
    const statusVariant = (row.subscriptionStatus || "Inactive")
      .toLowerCase()
      .replace(/\s+/g, "-");
    ...
    const product = {
      ...
      lastUpdatedText: row.lastUpdatedAt
  ...
  buildMetricCards(product) {
    return [
      this.buildMetric(
        "Synced Records",
        product.recordSyncCount,
        product.recordSyncLimit,
        "utility:database"
      ),
      this.buildMetric("Errors", product.errorCount, null, "utility:error"),
      this.buildMetric(
        "Endpoint Usage",
        product.endpointUsageCount,
        product.endpointUsageLimit,
  ...
  buildMetric(label, value, limit, icon) {
    ...
    const hasLimit =
      numericLimit !== null && numericLimit > 0 && numericValue !== null;
    const percent = hasLimit
      ? Math.min(100, Math.round((numericValue / numericLimit) * 100))
      : 0;
    ...
    return {
      ...
      limitText: hasLimit
        ? `${this.formatNumber(numericValue)} of ${this.formatNumber(numericLimit)} used`
        : ""
    };
  }
  ...
  isSubscribedProduct(product) {
    ...
    return (
      ...
      product.subscriptionStatus === "Expiring Soon"
    );
  }
  ...
  normalizeKeyList(value) {
    if (Array.isArray(value)) {
      return value
        .map((item) => this.normalizeProductKey(item))
        .filter(Boolean);
    }

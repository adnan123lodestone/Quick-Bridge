...
export default class IntegrationWorkAdmin extends LightningElement {
  ...
  async loadRows() {
    this.isLoading = true;
    try {
      this.rows = await getRecentWorkItems({
      ...
      this.toast(
        "Error",
        error?.body?.message || error.message || "Unable to load work items.",
        "error"
      );
    } finally {
  ...
  async handleRowAction(event) {
    ...
    if (actionName === "error") {
      this.toast(
        "Last Error",
        row.Last_Error__c || "No error recorded.",
        "info"
      );
      return;
    }
    ...
    try {
      ...
      this.toast(
        "Action Failed",
        error?.body?.message || error.message || "Work item action failed.",
        "error"
      );
    } finally {
      this.isLoading = false;

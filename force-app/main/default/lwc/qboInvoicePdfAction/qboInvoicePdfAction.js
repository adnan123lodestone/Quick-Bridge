...
export default class QboInvoicePdfAction extends LightningElement {
    ...
    loadPanel() {
        ...
        getPanelData({ recordId: this.recordId, objectApiName: this.objectApiName })
            ...
            .catch((error) => {
                ...
                this.panelMessage = error.body?.message || error.message || 'Failed to load QBO Invoice PDF panel.';
            })
            .finally(() => {
    ...
    get isDownloadDisabled() {
        return this.isLoading || this.isSyncingTo || this.isSyncingFrom || !this.qboIdPresent;
    }
    ...
    handleSyncToQbo() {
        ...
        syncToQbo({ recordId: this.recordId, objectApiName: this.objectApiName })
            .then((result) => {
                this.showToast('Success', result.message || 'Record synced to QBO.', 'success');
                this.loadPanel();
                this.dispatchEvent(new RefreshEvent());
            })
            .catch((error) => {
                this.showToast('Error', error.body?.message || error.message || 'Sync to QBO failed.', 'error');
            })
            .finally(() => {
    ...
    handleSyncFromQbo() {
        ...
        syncFromQbo({ recordId: this.recordId, objectApiName: this.objectApiName })
            .then((result) => {
                this.showToast('Success', result.message || 'Sync from QBO queued.', 'success');
                this.dispatchEvent(new RefreshEvent());
            })
            .catch((error) => {
                this.showToast('Error', error.body?.message || error.message || 'Sync from QBO failed.', 'error');
            })
            .finally(() => {
    ...
    handleDownload() {
        ...
        downloadPdf({ recordId: this.recordId, objectApiName: this.objectApiName })
            .then((result) => {
                this.showToast('Success', result.message || 'PDF saved to Files.', 'success');
            })
            .catch((error) => {
                this.showToast('Error', error.body?.message || error.message || 'PDF download failed.', 'error');
            })
            .finally(() => {

...
export default class QuickbridgeConfigPanel extends LightningElement {
  ...
  allTilesDefinition = [
    {
      ...
      label: "QuickBooks",
      logoUrl: QB_Logo,
      aliases: TILE_PROVIDER_ALIASES.qbo
    },
    {
      id: "shopify",
      productKey: "shopify",
      ...
      aliases: TILE_PROVIDER_ALIASES.shopify
    },
    {
      id: "stripe",
      productKey: "stripe",
      ...
      aliases: TILE_PROVIDER_ALIASES.stripe
    },
    {
      id: "authorizenet",
      productKey: "authorizenet",
      label: "Authorize.Net",
      logoUrl: Authorize_Net_logo,
      aliases: TILE_PROVIDER_ALIASES.authorizenet
    },
    {
      id: "paypal",
      productKey: "paypal",
      ...
      aliases: TILE_PROVIDER_ALIASES.paypal
    },
    {
      id: "fedex",
      productKey: "fedex",
      label: "FedEx",
      logoUrl: FedEx_Logo,
      aliases: TILE_PROVIDER_ALIASES.fedex
    },
    {
      id: "ups",
      productKey: "ups",
  ...
  isTileSubscriptionExpired(tileId, config) {
    ...
    const tile = this.getTileDefinition(tileId);
    const expiryFields = tile?.expiryField
      ? [tile.expiryField]
      : TILE_EXPIRY_FIELDS[tileId] || [];
  ...
  isStartDateAfterEndDate(tileId, config) {
    ...
    const fields = config.fields || config.formValues || {};
    const tile = this.getTileDefinition(tileId);
    const startFields = tile?.startField
      ? [tile.startField]
      : TILE_START_FIELDS[tileId] || [];
    const expiryFields = tile?.expiryField
      ? [tile.expiryField]
      : TILE_EXPIRY_FIELDS[tileId] || [];
    ...
    try {
      ...
      const tiles = (descriptors || [])
        ...
        .map((connector) => ({
          ...
          logoUrl:
            connector.logoUrl ||
            logoById[connector.connectorKey] ||
            QuickBridge_Logo,

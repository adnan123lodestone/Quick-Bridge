  ...
  async loadPanel() {
    ...
    try {
      ...
      this.selectedGateway =
        data?.selectedGateway || this.paymentOptions[0]?.value || "";
      this.panelMessage = data?.message || "";
    } catch (error) {
      this.paymentOptions = [];
      this.selectedGateway = "";
      this.panelMessage = this.getErrorMessage(
        error,
        "Failed to load invoice action."
      );
      this.showToast(
        "Unable to Load Invoice Action",
        this.panelMessage,
        "error"
      );
    } finally {
  ...
  async handleCreateInvoice() {
    ...
    try {
      ...
      this.showToast(
        ...
        this.getErrorMessage(error, "Invoice could not be created."),
        "error"
      );
    } finally {

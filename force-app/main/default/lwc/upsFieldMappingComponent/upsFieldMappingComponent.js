...
export default class UpsFieldMappingComponent extends LightningElement {
    ...
    return [
      {
        label: "SF to UPS",
        value: "SF to UPS",
        selected: this.selectedDirection === "SF to UPS"
      },
      {
        label: "UPS to SF",
        value: "UPS to SF",
        selected: this.selectedDirection === "UPS to SF"
      }
    ];
  }
  ...
  buildMappingRows(savedMappings) {
    ...
    const requiredUPSFields = this.upsFieldOptions.filter(
    ...
    relevantSavedMappings.forEach((mapping) => {
      const isRequired = requiredUPSFields.some(
  ...
  handleExternalFieldChange(event) {
    ...
    this.mappingRows = this.mappingRows.map((row) => {
      ...
      const upsField = this.upsFieldOptions.find(
      ...
      const keepSfField =
        upsField && sfField && this.isTypeMatch(sfField.type, upsField.type);
      return {
        ...row,
        externalField: value,
  ...
  handleSave() {
    ...
    if (duplicateExternalFields.length) {
      this.showToast(
        "Validation Error",
        "Each UPS field can only be mapped once per direction.",
        "error"
      );
      return;
    }
    ...
    if (missingRequiredRow) {
      this.showToast(
        "Validation Error",
        "Map a Salesforce field to every required UPS field.",
        "error"
      );
      return;
    }
    ...
    this.mappingRows = this.mappingRows.map((row) => {
      ...
      const availableSfOptions = upsType
        ? this.sfFieldOptions.filter((option) =>
            this.isTypeMatch(option.type, upsType)
          )
        : this.sfFieldOptions;

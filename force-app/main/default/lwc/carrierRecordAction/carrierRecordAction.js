...
export default class CarrierRecordAction extends LightningElement {
  ...
  loadActiveCarriersAndPanel() {
    ...
    getActiveCarriers()
      .then((carriers) => {
        const options = [];
        if (carriers.fedexActive)
          options.push({ label: "FedEx", value: "FedEx" });
    ...
    applyValidatedAddress({
      ...
      .then((res) => {
        if (res.success) {
          this.showToast(
            "Success",
            "Validated address applied to record.",
            "success"
          );
        } else {
          this.showToast(
  ...
  get confirmedAddressLines() {
    ...
    const fields = [
      ...
      {
        key: "State",
        value: addr.stateOrProvinceCode || addr.STATEORPROVINCECODE || null
      },

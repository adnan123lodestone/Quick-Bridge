...
export default class FedexFieldMappingComponent extends LightningElement {
    ...
    reloadMappings() {
        ...
        return Promise.all([
            ...
            getFedExFields({ actionName: this.selectedAction, direction: this.selectedDirection }),
            getExistingFedExMappings({ actionName: this.selectedAction, sfObject: this.selectedSFObject })
        ])
    ...
    buildMappingRows(savedMappings) {
        ...
        const requiredFedExFields = this.fedexFieldOptions.filter((field) => field.required);
        const rows = [];
        let counter = 1;
        ...
        relevantSavedMappings.forEach((mapping) => {
            const isRequired = requiredFedExFields.some((field) => field.value === mapping.externalField);
            if (!isRequired) {
                rows.push({
                    id: counter++,
    ...
    handleExternalFieldChange(event) {
        ...
        this.mappingRows = this.mappingRows.map((row) => {
            ...
            const fedexField = this.fedexFieldOptions.find((option) => option.value === value);
            const sfField = this.sfFieldOptions.find((option) => option.value === row.sfField);
            const keepSfField = fedexField && sfField && this.isTypeMatch(sfField.type, fedexField.type);
            return {
                ...row,
                externalField: value,
    ...
    handleSave() {
        ...
        if (duplicateExternalFields.length) {
            this.showToast('Validation Error', 'Each FedEx field can only be mapped once per direction.', 'error');
            return;
        }

        const missingRequiredRow = this.mappingRows.find((row) => row.isMandatory && (!row.externalField || !row.sfField));
        if (missingRequiredRow) {
            this.showToast('Validation Error', 'Map a Salesforce field to every required FedEx field.', 'error');
            return;
        }
    ...
    updateRowDropdowns() {
        this.mappingRows = this.mappingRows.map((row) => {
            const fedexField = this.fedexFieldOptions.find((option) => option.value === row.externalField);
            const fedexType = fedexField ? fedexField.type : null;
            const availableSfOptions = fedexType
                ? this.sfFieldOptions.filter((option) => this.isTypeMatch(option.type, fedexType))
                : this.sfFieldOptions;
